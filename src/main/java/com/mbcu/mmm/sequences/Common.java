...
public class Common extends Base {
	...
	private void filterTx(String raw) {
		...
		ArrayList<Offer> myOfferCreateds = new ArrayList<>();
		ArrayList<Offer> executedOffers = new ArrayList<>();
		...
		for (AffectedNode deletetedNode : deletedNodes) {
			LedgerEntry le = (LedgerEntry) deletetedNode.nodeAsFinal();
		...
		default:
			if (!myOfferCreateds.isEmpty()) {
				System.out.println("My Created Offers");

				myOfferCreateds.forEach(myOC -> {
					System.out.println(myOC.prettyJSON());
				});
				...
				if (!myExecutedOffers.isEmpty()){
					myExecutedOffers.forEach(myOC -> { // if other takes our order
						// TakePays and Taker Gets should be 0 for fully consumed
						System.out.println("MY ORDER IS TAKEN ");
						// In this case there is no new order created and seq doesn't change
					...
					executedOffers.forEach(oc -> {
						// This order is seen from the other guy's perspective
						// if FinalField's takerPays and TakerGets == 0 then we fully consume the guy's order
